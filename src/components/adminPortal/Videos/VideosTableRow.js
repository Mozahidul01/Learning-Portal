...
export default function VideosTableRow({ video }) {
  const { id, title, description } = video || {};

  const [deleteVideo, { isLoading, isError }] = useDeleteVideoMutation();
  ...
  return (
    <tr>
      ...
      <td className="table-td">{truncateText(description, 20)}</td>
